...
- **Logistic Regression with Best Subset Selection:** Exhaustive search over all possible combinations of features based on cross-validation accuracy to determine the best subset of wine features and logistic regression will then be performed on this subset.
...
Each model will undergo a rigorous validation process, likely employing techniques such as K-folds cross-validation and various performance metrics (accuracy, F1 score, confusion matrix, etc.) 
...
For the first model, we implemented a Logistic Regression combined with Best Subset Selection. The Best Subset Selection method enhances model performance by exhaustively searching for the optimal combination of predictors that yield the highest cross-validation accuracy. This model utilizes a robust set of nine predictors: `fixed acidity`, `volatile acidity`, `citric acid`, `residual sugar`, `chlorides`, `free sulfur dioxide`, `total sulfur dioxide`, `sulphates`, and `alcohol`. We integrate Stratified K-Fold cross-validation (K=4) to ensure that each class is evenly represented during model training, thereby maintaining model generalizability and preventing class imbalance.

The classification performance of our Logistic Regression model shows a weighted average F1-score of 0.61, indicating a fair level of predictive accuracy, particularly for mid-range classes. However, the model shows some limitations in predicting the extreme classes (categories 3, 4, 7, and 8) as evidenced by lower F1-scores and precision-recall metrics. This issue could be attributed to the smaller number of training samples available for these categories, which affects the model's ability to generalize well across all classes. Despite these challenges, the model achieves a reasonable overall accuracy of 63%, highlighting its utility in multi-class predictions but also pointing to areas where further model tuning and training data enrichment could improve performance.
